...
import static cn.edu.xidian.xidian3d.util.Util.readIntLe;
    ...
    private void readBinary(@NonNull BufferedInputStream inputStream) throws IOException {
        ...
        if (!haveNormals) {
            ...
            for (int i = 0; i < vertexCount; i += 3) {
                Util.calculateNormal(vertexArray[i * 3], vertexArray[i * 3 + 1], vertexArray[i * 3 + 2], vertexArray[(i + 1) * 3], vertexArray[(i + 1) * 3 + 1], vertexArray[(i + 1) * 3 + 2], vertexArray[(i + 2) * 3], vertexArray[(i + 2) * 3 + 1], vertexArray[(i + 2) * 3 + 2], customNormal);
